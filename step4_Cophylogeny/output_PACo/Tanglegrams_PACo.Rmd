...
```{r}
tree_bat <- read.newick("bat_besttree_withConfident.nwk", node.label = "support")
tree_virus <- read.newick("V_Best_tree_with_confidence.nwk",node.label = "support")
```
...
plot_tree_bat <- ggtree(tree_bat) +
  ...
  theme_tree2()  + xlim(NA,2.5) + geom_text(aes(label=support1),hjust=-0.5,size = 3,face = "bold") +
...
plot_tree_virus2 <- ggtree(tree_virus) +
  ...
  theme_tree2() + geom_text(aes(label=support2),hjust=-0.3,size = 3,face = "bold", nudge_y = 0.5,  nudge_x = -0.01) +
  theme(legend.position = "none")
